...
@click.option("-l", "--labels", default=None, help="Comma-separated label space (overrides dataset default)")
@click.option("-p", "--provider", default="anthropic", help="LLM provider: anthropic, openai, groq, ollama")
...
@click.option("--label-model", default="generative", help="Label model: majority, weighted, generative")
...
@click.option("--small-model", is_flag=True, default=False, help="Optimize for small models (8B and below)")
def run(
    dataset: str,
    task: str | None,
    labels: str | None,
...
def benchmark(dataset: str, provider: str, model: str | None, max_iterations: int) -> None:
    ...
    llm = get_provider(
        name=provider,
        model=model or "",
        api_key=config.anthropic_api_key if provider == "anthropic" else config.openai_api_key,
    )
    ...
    runner.run(datasets=datasets, max_iterations=max_iterations)
